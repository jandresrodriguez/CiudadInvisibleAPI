  ...
  def preferences_posts
    begin
      ...
      if preferences_posts.empty?
        ...
        render json: preferences_posts.to_json(:include => { :assets => {:only => [:file_file_name, :file_content_type],:methods => :file_url }} , :methods => [:first_image, :favorites_quantity, :comments_quantity]), status: :ok
      end
    rescue
      render json: "error", status: :unprocessable_entity
    end
    ...
    begin
      ...
      if params[:latitude] && params[:longitude] && params[:user_id]
        ...
        unless nearby_posts.empty?
          ...
          puts "19"
          render json: tour.part_of_tours.order("tour_order").to_json(include: :post), status: :ok
        else
          puts "20"
